...
def check_token_existence(token_to_check):
    ...
    if token_to_check in token_data['tokens']:
        ...
        print(f"a|Token SAVE {token_symbol}", f"[{token_to_check}] already exists in 'alreadyBoughtTokens.json'.")
        return True
    return False
...
def storeSettings(amm,
    ...
    print(f"a|Token INFO SAVE {token_symbol}", f"Settings saved in 'previousSELLBUYINFO.json'.")
