...
def BuyToken(TOKEN_TO_SWAP_BUY, payer, amount):
    with CruserSolana() as solana_client:
        ...
        if pool_keys == "failed":
            ...
            return {
                "status": 404,
                "message": "The pool you’re looking for may not be available yet."
            }
            time.sleep(0.500)
            # BuyToken(TOKEN_TO_SWAP_BUY, payer, amount)
        """
        Calculate amount
        """
...
def BuyTokenSniper(TOKEN_TO_SWAP_BUY, payer, amount):
    with CruserSolana() as solana_client:
        token_symbol, SOl_Symbol = getSymbol(TOKEN_TO_SWAP_BUY)
        mint = Pubkey.from_string(TOKEN_TO_SWAP_BUY)
        pool_keys = fetch_pool_keys(str(mint))
        ...
        if pool_keys == "failed":
            print(f"a|BUY Pool ERROR {token_symbol}",f"[Raydium]: Pool Key Not Found")
            time.sleep(0.500)
            return BuyToken(TOKEN_TO_SWAP_BUY, payer, amount)
            # return {
            #     "status": 404,
            #     "message": "The pool you’re looking for may not be available yet."
            # }
