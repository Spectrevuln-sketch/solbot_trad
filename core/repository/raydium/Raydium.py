...
from core.utils.alreadyBought import soldToken

from core.repository.raydium.sell_swap import sell
from core.utils.birdeye import get_price, getSymbol
from core.utils.monitor_price_strategy import limit_order, trailing_stop_loss_func, take_profit_and_trailing_stop
...
def raydium_swap(config, ctx, payer, desired_token_address, txB, execution_time, limit_order_sell_Bool, take_profit_ratio, trailing_stop_Bool, trailing_stop_ratio, Limit_and_Trailing_Stop_Bool, bought_token_price):
    ...
    if sell_NOW == False:
        ...
        if str(txS) != 'failed':
            ...
            return {
                'status': 200,
                'message': 'Successfully sold token'
            }
        else:
            return {
                'status': 400,
                'message': 'Filed sold token failed'
            }