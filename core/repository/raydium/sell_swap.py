...
from core.repository.raydium.create_close_account import  fetch_pool_keys, sell_get_token_account,get_token_account, make_swap_instruction
from core.utils.birdeye import getSymbol
from core.connect_solana import CruserSolana, SolThrad, SolanaCrypt
...
def sellCurrentToken(TOKEN_TO_SWAP_SELL, payer, amount):
    with CruserSolana() as solana_client:
        ...
        if pool_keys == "failed":
            ...
            return {
                "status": 404,
                "message": "Pool Key Not Found"
            }
        # Get token balance from wallet
        accountProgramId = solana_client.get_account_info_json_parsed(mint)
        programIdToken = accountProgramId.value.owner
        print(f'Token Mint {mint}')
        accounts = solana_client.get_token_accounts_by_owner_json_parsed(payer.pubkey(),TokenAccountOpts( program_id=programIdToken)).value
        ...
        # return print(f'WSOL TOKEN ACCOUNT: %s' % WSOL_token_account)

        if swap_token_account == None:
            print("swap_token_account not found...")
            return {
                "status": 400,
                "message": "Swap token not found"
            }

        else:
            """Make swap instructions"""
            amount_to_sell = (int(amount) / 100 ) * amount_in
                ...
                while checkTxn:
                    try:
                        ...
                        if status.value.transaction.meta.err == None:
                            ...
                            return{
                                'status': 400,
                                'message': 'Transaction Failed Execution time: {execution_time}'
                            }
                            print(f"Execution time: {execution_time} seconds")
