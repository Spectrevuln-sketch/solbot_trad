...
# Connect Wallet data
  ...
  @classmethod
  def SnipeToken(cls, tele_id, token_address):
      wallet = GetCurrentWallet(tele_id=tele_id)
      print(f'Wallet data  ====== {wallet}')
      cls.CheckLunchToken(token_address)
      secretByte = base58.b58decode(wallet[2])
      payer = Keypair().from_bytes(secretByte)
      with SolThrad() as event_thread:
              Thread(target=select_amm2trade, name=token_address, args=(
                  token_address, payer)).start()
              event_thread.wait()
              event_thread.clear()
