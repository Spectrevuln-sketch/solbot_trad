...
def ButtonCallback (update: telegram.Update, context: CallbackContext):
    ...
    #   Change WAllet Button
          ...
          if payload[SNIPE_MODE]:
              query.message.reply_text("Start snipe mode")
              snapToken = SolanaHandler().SnipeToken(payload["wallet"], payload["token_address"], float(convertQuery))
              if snapToken['status']:
                  query.message.reply_text(snapToken["message"])
        #   ammPDA = getAccountPool(payload["token_address"])
        #   print(f'AMM PDA {ammPDA}')
          query.message.reply_text("Start buy mode")
          ...
          query.message.reply_text("Start sell mode")
